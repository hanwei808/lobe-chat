...
Upgrading the Docker deployment version is very simple, just redeploy the latest image of WodeChat. Here are the instructions to perform these steps:

1. Stop and delete the currently running WodeChat container (assuming the name of the WodeChat container is `lobe-chat`):
...
2. Pull the latest Docker image of WodeChat:

```fish
docker pull lobehub/lobe-chat
```

3. Redeploy the WodeChat container using the newly pulled image:
...
>
> Don't worry, all of WodeChat's chat history is stored in your local browser. Therefore, when you redeploy WodeChat using Docker, your chat history will not be lost.

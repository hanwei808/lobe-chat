---
title: Deploy WodeChat with database on Dokploy
description: >-
  Learn how to deploy WodeChat with database on Dokploy with ease, including: database, authentication and S3 storage service.
...
This article will detail how to deploy the server database version of WodeChat.
...
Enter the created database and set an unused port in External Credentials to allow external access; otherwise, WodeChat will not be able to connect to the database. You can view the Postgres database connection URL in External Host, as shown below:
...
## Deploy WodeChat on Dokploy.

Click "Create Service", select "Application", and create the WodeChat application.

![](https://github.com/user-attachments/assets/4cbbbcce-36be-48ff-bb0b-31607a0bba5c)

Enter the created WodeChat application, select the forked lobe-chat project and branch, and click Save to save.
...
After a successful deployment, bind your own domain to your WodeChat application and request a certificate on the Domains page.

![](https://github.com/user-attachments/assets/dd6bc4a4-3c20-4162-87fd-5cac57e5d7e7)

## Check if WodeChat is working properly.

Go to your WodeChat website, and if you click on the login button in the upper left corner and the login pop-up appears normally, it means you have configured it successfully. Enjoy it to the fullest!

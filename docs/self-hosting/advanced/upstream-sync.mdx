---
title: WodeChat Upstream Sync and Docker Deployment Guide
description: >-
  Learn how to enable automatic updates for WodeChat Vercel deployments and upgrade Docker deployment versions easily. Follow step-by-step instructions for a seamless deployment process.
...
Upgrading the Docker deployment version is very simple, you just need to redeploy the latest WodeChat image. Here are the commands required to perform these steps:

<Steps>
  ### Stop and Remove the Current Running WodeChat Container

  Assuming the WodeChat container is named `lobe-chat`, use the following commands to stop and remove the currently running WodeChat container:
  ...
  ### Pull the Latest WodeChat Image

  Use the following command to pull the latest Docker image for WodeChat:
  ...
  Redeploy the WodeChat container using the newly pulled image:
...
<Callout type={'tip'}>
  **If I redeploy, will I lose my local chat records?**
  ...
  No need to worry, you won't. All of WodeChat's chat records are stored in your local browser. Therefore, when redeploying WodeChat using Docker, your chat records will not be lost.
</Callout>

---
title: Configuring Authelia Authentication Service for WodeChat
description: >-
  Learn how to configure Authelia authentication service in WodeChat, including creating a provider, configuring environment variables, and deploying WodeChat. Detailed steps and necessary environment variable settings.
...
<Steps>
  ...
  Make sure to replace secret and `redirect_urls` with your own values. Note! The secret configured in Authelia is ciphertext, i.e., a salted hash value. Its corresponding plaintext needs to be filled in WodeChat later.

---
title: 在 WodeChat 中配置 Casdoor 身份验证服务
description: 学习如何在 WodeChat 中配置 Casdoor 身份验证服务，包括部署、创建、设置权限和环境变量。
tags:
  - Casdoor 身份验证
  - 环境变量配置
  - 单点登录
...
<Callout type={'tip'}>
  若你想要私有部署 Casdoor，我们建议你将之与 WodeChat 数据库版本一同使用 Docker Compose 部署，此时
  WodeChat 可以与之共用同一个 Postgres 实例。
</Callout>
...
- 你的 WodeChat 数据库版本 IP / 端口为 `http://LOBECHAT_IP:3210`。
- 你私有部署 Casdoor，其域名为 `http://CASDOOR_IP:8000`。

若你使用公网部署，下文假设：

- 你的 WodeChat 数据库版本域名为 `https://lobe.example.com`。
...
<Steps>
  ...
  前往 `身份认证` -> `应用`，创建一个 `WodeChat` 应用或直接修改内置的 `built-in` 应用，其他字段可以自行探索，但你至少需要配置以下字段：
  ...
  配置 Casdoor 的 Webhook 以便在用户信息更新时同步到 WodeChat 。
  ...
  将获取到的 `客户端 ID` 和 `客户端`，设为 WodeChat 环境变量中的 `AUTH_CASDOOR_ID` 和 `AUTH_CASDOOR_SECRET`。

  配置 WodeChat 环境变量中 `AUTH_CASDOOR_ISSUER` 为：

  - `http://localhost:8000/`，若你是本地开发环境
  - `http://CASDOOR_IP:8000/`，若你是局域网私有部署的 Casdoor
  - `https://lobe-auth-api.example.com/`，若你是公网环境部署的 Casdoor

  在部署 WodeChat 时，你需要配置以下环境变量：
...
<Callout type={'info'}>部署成功后，用户将可以通过 Casdoor 身份认证并使用 WodeChat。</Callout>

---
title: 在 Dokploy 上部署 WodeChat 的服务端数据库版本
description: 本文详细介绍如何在 Dokploy 中部署服务端数据库版 WodeChat，包括数据库配置、身份验证服务配置的设置步骤。
tags:
  - 服务端数据库
  - Postgres
  - Clerk
...
本文将详细介绍如何在 Dokploy 中部署服务端数据库版 WodeChat。
...
进入创建的数据库，在 External Credentials 设置一个未被占用的端口，使其能能通过外部访问，否则 WodeChat 将无法连接到该数据库。你可以在 External Host 查看 Postgres 数据库连接 URL ，如下：
...
## 在 Dokploy 上部署 WodeChat

点击 Create Service 选择 Application，创建 WodeChat 应用

![](https://github.com/user-attachments/assets/4cbbbcce-36be-48ff-bb0b-31607a0bba5c)

进入创建的 WodeChat 应用，选择你 fork 的 lobe-chat 项目及分支，点击 Save 保存
...
部署成功后在 Domains 页面，为你的 WodeChat 应用绑定自己的域名并申请证书。

![](https://github.com/user-attachments/assets/dd6bc4a4-3c20-4162-87fd-5cac57e5d7e7)

## 验证 WodeChat 是否正常工作

进入你的 WodeChat 网址，如果你点击左上角登录，可以正常显示登录弹窗，那么说明你已经配置成功了，尽情享用吧～

---
title: Configuring Logto Authentication Service in WodeChat
description: >-
  Learn how to configure Logto authentication service in WodeChat, including deployment, creation, setting permissions, and environment variables.
...
<Callout type={'tip'}>
  ...
  with the WodeChat database version. In this case, WodeChat can share the same Postgres instance
  with it.
</Callout>

## Logto Configuration Process

The following assumes your WodeChat database version domain is `https://lobe.example.com`.
...
<Steps>
  ...
  Configure the Logto Webhook so that WodeChat can receive notifications when user information is updated.
  ...
  Set the obtained `Client ID` and `Client Secret` as `AUTH_LOGTO_ID` and `AUTH_LOGTO_SECRET` in the WodeChat environment variables.

  Configure `AUTH_LOGTO_ISSUER` in the WodeChat environment variables as follows:
  ...
  | `NEXT_AUTH_SSO_PROVIDERS`   | Required | Select the single sign-on provider for WodeChat. For Logto, enter `logto`.                                                                                                             |
...
<Callout type={'info'}>
  After successful deployment, users will be able to authenticate via Logto and use WodeChat.
</Callout>

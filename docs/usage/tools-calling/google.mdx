...
description: >-
  使用 WodeChat 测试 Google Gemini 系列模型（Gemini 1.5 Pro / Gemini 1.5 Flash） 的工具调用（Function Calling）能力，并展现评测结果
...
In the json output from Gemini, the name is incorrect, so WodeChat cannot recognize which plugin it called. (In the input, the name of the weather plugin is `realtime-weather____fetchCurrentWeather`, while Gemini returns `weather____fetchCurrentWeather`).

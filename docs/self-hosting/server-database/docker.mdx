---
title: Deploying WodeChat Database with Docker
description: >-
  Learn how to deploy the WodeChat server database version using Docker on Linux and local machines.
...
<Callout type="warning">
  Due to the inability to expose `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` using Docker environment variables, you cannot use Clerk as an authentication service when deploying WodeChat using Docker / Docker Compose.
...
Here is the process for deploying the WodeChat server database version on a Linux server:
...
<Steps>
  ...
  <Callout type="info">
    ...
    component for WodeChat to implement RAG.
  </Callout>
  ...
  S3_BUCKET=wodechat
  S3_PUBLIC_DOMAIN=https://s3-for-wodechat.your-domain.com
...
The data version of WodeChat also supports direct use on a local Mac/Windows machine.
...
$ docker run -it -d --name lobe-chat-database -p 3210:3210 \
  ...
  -e S3_BUCKET=wodechat \
  -e S3_PUBLIC_DOMAIN=https://s3-for-wodechat.your-domain.com \
